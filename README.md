...
現場向けの使い方は [MANUAL.md](MANUAL.md) にある。以下は構成と設置の説明。
...
全員に同じアクセストークンを配り、②の「実施者」欄に各自の名前を入れてもらう。誰がやったかは記録タブで追える。

`ADMIN_TOKENS` を設定すると、**統合（ステップ3）と削除**が管理者だけになる。未設定のあいだは全員が実行できる。
管理者トークンは `ACCESS_TOKENS` にも **同じラベル** で登録すること。ラベルが違うとデータの持ち主が別扱いになり、
管理者から他の人のデータが見えなくなる。

```
ACCESS_TOKENS = "clarion:みんなの共有トークン,clarion:管理者トークン"
